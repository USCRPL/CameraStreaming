...
Dword Cmd_busTx (
    ...
    for (i = 0; i < User_RETRY_MAX_LIMIT; i++) {
        error = it9130->cmdDescription->busDescription->busTx (demodulator, bufferLength, buffer);
...
Dword Cmd_busRx (
    ...
    for (i = 0; i < User_RETRY_MAX_LIMIT; i++) {
        error = it9130->cmdDescription->busDescription->busRx (demodulator, bufferLength, buffer);
        if (error == 0) goto exit;
...
Dword Cmd_busRxData (
    ...
    for (i = 0; i < User_RETRY_MAX_LIMIT; i++) {    
        error = it9130->cmdDescription->busDescription->busRxData (demodulator, bufferLength, buffer);
        if (error == 0) goto exit;
...
Dword Cmd_writeRegisters (
    ...
    it9130     = (IT9130*) demodulator;
    maxFrameSize = it9130->cmdDescription->mailBoxSize; /** releaseExternalRemove */
