 ...
 *     error = Demodulator_writeEepromValues ((Demodulator*) &it9130, 0, 0x0000, 3, buffer);
 *     if (error)
 *         printf ("Error Code = %X", error);
 *     else
 *         printf ("Success");
 ...
 *     error = Demodulator_readEepromValues ((Demodulator*) &it9130, 0, 0x0000, 3, buffer);
 *     if (error)
 *         printf ("Error Code = %X", error);
 *     else
 *         printf ("Success");
 ...
 *
 *     // Get the type of interrupts.
 *     error = Demodulator_getInterrupts ((Demodulator*) &it9130, &interrupts);
 *     if (error)
 *         printf ("Error Code = %X", error);
 *     else
 *         printf ("Success");
 ...
 *
 *     // Set statistic range.
 *     error = Demodulator_clearInterrupt ((Demodulator*) &it9130, Interrupt_SIPSI);
 *     if (error)
 *         printf ("Error Code = %X", error);
 *     else
 *         printf ("Success");
 ...
 * @param chip version.
 * @return Error_NO_ERROR: successful, non-zero error code otherwise.
 * @return Error_INVALID_CHIP_TYPE: if the chip type Unrecognized.
 * @return Error_TUNER_TYPE_NOT_COMPATIBLE: if the tuner type Unrecognized.
 */
Dword Demodulator_checkChipType (
	IN  Demodulator*    demodulator,
	IN  Byte            chip,
	OUT Dword*			chip_Type,
...
*/
Dword Demodulator_setDcaLatchTime(
	IN  Demodulator*    demodulator,
	IN  Byte            masterValue,
	IN  Byte            slaveValue
);
...
*        T-DMB: 5000, 6000, 7000, and 8000 (KHz).
*        FM: 100, and 200 (KHz).
* @param masterFreq the master channel frequency in KHz.
* @param slaveFreq the slave channel frequency in KHz.
* @return Error_NO_ERROR: successful, non-zero error code otherwise.
*/
Dword Demodulator_acquireChannelEx(
	IN  Demodulator*    demodulator,
	IN	Word			bandwidth,
	IN  Dword           masterFreq,
	IN  Dword           slaveFreq
