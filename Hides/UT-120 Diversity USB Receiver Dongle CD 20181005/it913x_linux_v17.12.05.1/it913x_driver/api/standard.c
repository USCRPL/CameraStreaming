...
Dword Standard_sendCommand (
	...
	if (it9130->cmdDescription->sendCommand != NULL) {
		error = it9130->cmdDescription->sendCommand (demodulator, command, chip, processor, writeBufferLength, writeBuffer, readBufferLength, readBuffer);
	}
#endif
	return (error);
}
...
Dword Standard_loadFirmware (
	...
	for (i = beginPartition; i < endPartition; i++) {
		...
		if (firmwareSegments[i].segmentType == 0) {
			...
			if (error) goto exit;
			if (it9130->cmdDescription->loadFirmware != NULL) {
				error = it9130->cmdDescription->loadFirmware (demodulator, firmwareLength, firmwareCodesPointer);
			}
			if (error) goto exit;
...
Dword Standard_writeRegisters (
	...
	if (it9130->cmdDescription->writeRegisters != NULL) {
		error = it9130->cmdDescription->writeRegisters (demodulator, chip, processor, registerAddress, registerAddressLength, bufferLength, buffer);
	}
#endif
...
Dword Standard_writeEepromValues (
	...
	/** Read EEPROM address. */
	/*error = it9130->cmdDescription.readRegisters (demodulator, chip, Processor_LINK, 0xBFF0, 1, &eepromAddress);
	if (error) goto exit;
	*/
	eepromAddress = 0x01;

	/** Read EEPROM valid length of register. */
	/*error = it9130->cmdDescription.readRegisters (demodulator, chip, Processor_LINK, 0xBFF1, 1, &registerAddressLength);
	if (error) goto exit;
	*/
	registerAddressLength = 0x01;

	if (it9130->cmdDescription->writeEepromValues != NULL) {
		error = it9130->cmdDescription->writeEepromValues (demodulator, chip, eepromAddress, registerAddress, registerAddressLength, bufferLength, buffer);
	}
...
Dword Standard_writeRegisterBits (
	...
	if (length == 8) {
		if (it9130->cmdDescription->writeRegisters != NULL) {
			error = it9130->cmdDescription->writeRegisters (demodulator, chip, processor, registerAddress, registerAddressLength, 1, &value);
		}
	} else {
		if (it9130->cmdDescription->readRegisters != NULL) {
			error = it9130->cmdDescription->readRegisters (demodulator, chip, processor, registerAddress, registerAddressLength, 1, &temp);
			if (error) goto exit;
		}
		...
		if (it9130->cmdDescription->writeRegisters != NULL) {
			error = it9130->cmdDescription->writeRegisters (demodulator, chip, processor, registerAddress, registerAddressLength, 1, &temp);
			if (error) goto exit;
		}
	}
exit:
...
Dword Standard_readRegisters (
	...
	if (it9130->cmdDescription->readRegisters != NULL) {
		error = it9130->cmdDescription->readRegisters (demodulator, chip, processor, registerAddress, registerAddressLength, bufferLength, buffer);
	}
#endif
...
Dword Standard_readEepromValues (
	...
	*/
	...
	if (it9130->cmdDescription->readEepromValues != NULL) {
		error = it9130->cmdDescription->readEepromValues (demodulator, chip, eepromAddress, registerAddress, registerAddressLength, bufferLength, buffer);
	}
...
Dword Standard_loadIrTable (
	...
	error = Standard_readRegister (demodulator, 0, Processor_LINK, ir_table_start_15_8, &baseHigh);
	if (error) goto exit;
	error = Standard_readRegister (demodulator, 0, Processor_LINK, ir_table_start_7_0, &baseLow);
	if (error) goto exit;
...
Dword Standard_initialize (
	...
	it9130->driver = NULL;
	error = Standard_getDriver (demodulator, &it9130->driver);
...
Dword Standard_setArchitecture (
	...
	if ((it9130->busId == Bus_USB) || (it9130->busId == Bus_USB11)) {
		//below is for dynamic URB_BUF_SIZE
		frameSize = it9130->user_usb20_frame_size_dw;//User_USB20_FRAME_SIZE_DW;
...
Dword Standard_clearInterrupt (
	...
	//error = it9130->dvbtStandardDescription.clearInterrupt (demodulator, interrupt);
...
Dword Standard_getData (
	...
	//error = it9130->dvbtStandardDescription.getData (demodulator, bufferLength, buffer);
	...
	error = it9130->cmdDescription->receiveData (demodulator, 0xF00000, bufferLength, buffer);
	if (error) goto exit;
...
//	Bool ready = False;
	...
	//error = it9130->dvbtStandardDescription.getDatagram (demodulator, bufferLength, buffer);
	...
	if (*bufferLength >= length) {
		...
		/** IP datagram is locate in a special register 0xF00000 */
		error = it9130->cmdDescription->receiveData (demodulator, 0xF00000, length, buffer);
		if (error) goto exit;
...
//	Byte chip_version = 0;
	...
	if (version != 0) {
		//for (i = it9130->chipNumber; i > 0; i--) {
			error = it9130->cmdDescription->reboot (demodulator, 0);
...
Dword Standard_setBurstSize (
	...
	switch (burstSize) {
		case BurstSize_1024 :
			...
			it9130->burstSize = BurstSize_1024;
			break;
		case BurstSize_2048 :
			for (i = 0; i < it9130->chipNumber; i++) {
				error = Standard_writeRegisterBits (demodulator, i, Processor_LINK, p_reg_dvbt_path, reg_dvbt_path_pos, reg_dvbt_path_len, 1);
			...
			it9130->burstSize = BurstSize_2048;
		...
		case BurstSize_4096 :
			...
			it9130->burstSize = BurstSize_4096;
			break;
	}
...
Dword Standard_checkChipType (
	IN  Demodulator*    demodulator,
	IN  Byte            chip,
	OUT Dword*			chip_Type,
	OUT Byte*			chip_version	
) {
	Dword error = Error_NO_ERROR;
	...
	error = Standard_readRegister(demodulator, chip, Processor_LINK, chip_version_7_0, chip_version);
	if (error) goto exit;
	error = Standard_readRegisters(demodulator, chip, Processor_LINK, chip_version_7_0+1, 2, var);
	if (error) goto exit;
	*chip_Type = var[1]<<8 | var[0];	

	tunerId = it9130->tunerDescription->tunerId;

	if(!(*chip_Type==0x9035 || *chip_Type==0x9135 || *chip_Type==0x9175)){
		error = Standard_readRegisters(demodulator, chip, Processor_LINK, 0x1220, 3, var);
		if (error) goto exit;
		*chip_Type =var[2]<<16 | var[1]<<8 | var[0];
		if(*chip_Type == 0xf83602 || *chip_Type == 0x030180){
			*chip_Type = 0x9035;
			if(tunerId > 0x44 || tunerId == 0)
				error = Error_TUNER_TYPE_NOT_COMPATIBLE;

		}else{
			error = Error_INVALID_CHIP_TYPE;
		}	
	}else if(*chip_Type==0x9135){
		if(!((tunerId > 0x50 &&tunerId < 0x70) || tunerId == 0x38))
			error = Error_TUNER_TYPE_NOT_COMPATIBLE;
	}else if(*chip_Type==0x9175){
		if(tunerId < 0x70 || tunerId > 0x7F)
			error = Error_TUNER_TYPE_NOT_COMPATIBLE;
	}
exit :
	return (error);
...
Dword Standard_setDcaLatchTime(
	IN  Demodulator*    demodulator,
	IN  Byte            masterValue,
	IN  Byte            slaveValue
){
...
Dword Standard_acquireChannelEx(
	IN  Demodulator*    demodulator,
	IN	Word			bandwidth,
	IN  Dword           masterFreq,
	IN  Dword           slaveFreq
