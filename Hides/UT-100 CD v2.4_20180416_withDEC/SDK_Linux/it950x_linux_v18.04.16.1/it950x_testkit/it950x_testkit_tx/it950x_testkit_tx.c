...
#define EEPROM_DEVICETYPE_ADDRESS 0x41

//***************** IQTable Constant *****************//
#define IQTABLE_NROW 65536
#define Default_IQtable_Path "./bin/IQ_table.bin"

Bool g_TsPacket_HasParityCheck = False;
...
static Byte EEPROM_Default_Data[256] = {
...
Bool ReadFile_MyModifiedVersion(
    FILE *	TsFile,
    Byte	*fileBuf,
	Dword	Tx_datalength,
    ULONGLONG *bytesRead
...
{
	if(!g_TsPacket_HasParityCheck)
	{
//		ReadFile(TsFile, fileBuf, Tx_datalength, bytesRead, lpOverlapped);
		*bytesRead = fread(fileBuf, 1, Tx_datalength, TsFile);
	}
	else
	{
		Byte tempBuffer[204] = {0};
		unsigned long tempbytesRead = 0;
		*bytesRead = 0;
		Dword i=0;

		for(i=0; i<Tx_datalength/188; i++)
		{
//			ReadFile(TsFile, tempBuffer, sizeof(tempBuffer), &tempbytesRead, lpOverlapped);
			tempbytesRead = fread(tempBuffer, 1, sizeof(tempBuffer), TsFile);
			
			if( (tempBuffer[0] == 0x47) && 
				(tempBuffer[1] == 0x1F) && 
				(tempBuffer[2] == 0xFF)  
				)
			{
				if(tempbytesRead == sizeof(tempBuffer))
				{
					i--;
					continue;
				}
				else if(tempbytesRead < sizeof(tempBuffer))
					break;
			}
			...
			memcpy(fileBuf+(i*188) , tempBuffer, tempbytesRead);
			memset(tempBuffer, 0, sizeof(tempBuffer));
			*bytesRead += tempbytesRead;

			if(tempbytesRead < 188)
				break;
		}
	}
...
Bool HasParityCheck(Byte *fileBuf, ULONGLONG bytesRead, ULONGLONG *FirstSyncBytePos)
{
	ULONGLONG i = *FirstSyncBytePos;
	for (i = 0; (i < bytesRead - 204); i++) 
	{
			if ((fileBuf[i] == 0x47)&&(fileBuf[i+204] == 0x47))
			{
				//Sync byte found
...
//	dwReadSize = fread(pbTSBuffData, 1, len, TsFile);
	bRet = ReadFile_MyModifiedVersion(TsFile,pbTSBuffData,len,&dwReadSize);
	...
	for (FirstSyncBytePos=0;(FirstSyncBytePos<dwReadSize-188);FirstSyncBytePos++) 
	{
			if ((pbTSBuffData[FirstSyncBytePos] == 0x47)&&(pbTSBuffData[FirstSyncBytePos+188] == 0x47))
			{
				 //Sync byte found
				 printf("TS Sync byte found in offset:%llu\n",FirstSyncBytePos);
				g_TsPacket_HasParityCheck = False;
				 break;
			}
	}
	...
	if (FirstSyncBytePos>=dwReadSize-188) {
		...
		if(HasParityCheck(pbTSBuffData, dwReadSize, &FirstSyncBytePos))
		{
			printf("Every single TS packet has 16-bits Parity Check in this TS file\n");
			g_TsPacket_HasParityCheck = True;
		}
		else
		{
			printf("It's a invalid TS file, TX_DataOutputTest() Quit !!!\n");
			return 0;
		}		
	}
	...
	while (dwReadSize>188) 
		...
		for (i=0;(i<dwReadSize);i+=188) 
								...
								if (mPID==PID) 
								{
									...
									if (prev_pcr) {
										...
										if (pcr>prev_pcr)
											...
											pcr_diff=pcr+0x1FFFFFFFFLL-prev_pcr;
		    ...
		    if (FileOffset>10000000 && FileOffset<FileSize-10000000 && pcr2) 
			{
				...
				//dwReadSize = fread(pbTSBuffData, 1, len, TsFile);				
				ReadFile_MyModifiedVersion(TsFile,pbTSBuffData,len,&dwReadSize);
...
Dword TxDataOutputTest(ModulatorParam param, Byte DevNo)
{
	...
	fseek(TsFile, 0, SEEK_SET);	
//	bytesRead = fread(fileBuf, 1, Tx_datalength, TsFile);
	ReadFile_MyModifiedVersion(TsFile, fileBuf, Tx_datalength, &bytesRead);
	...
	if (FirstSyncBytePos>=bytesRead-188) {
		...
		if(HasParityCheck(fileBuf, bytesRead, &FirstSyncBytePos))
		{
			printf("Every single TS packet has 16-bits Parity Check in this TS file\n");
			g_TsPacket_HasParityCheck = True;
		}
...
static int RwRegister(Byte DevNo)
{
	int res = 0;
	...
	if (rwFlag == 1) { // Write register
		...
		if (processor == 0) {
			dwStatus = g_ITEAPI_TxWriteRegLINK((Word)regAddr, (Byte)vaule, DevNo);
			if (dwStatus)
				printf("g_ITEAPI_TxWriteRegLINK() return error!\n");
			else
				printf("\nWrite LINK Address 0x%X Value 0x%X OK.\n", regAddr, vaule);
		}
		else {
			dwStatus = g_ITEAPI_TxWriteRegOFDM((Word)regAddr, (Byte)vaule, DevNo);
			if (dwStatus)
				printf("g_ITEAPI_TxWriteRegOFDM() return error!\n");
			else
		...
		if (processor == 0) {
			dwStatus = g_ITEAPI_TxReadRegLINK((Word)regAddr, (Byte*) &vaule, DevNo);
			if (dwStatus)
				printf("g_ITEAPI_TxReadRegLINK() return error 0x%lX!\n", dwStatus);
			else
				printf("\nRead LINK Address 0x%X Value 0x%X OK.\n", regAddr, vaule);
		}
		else {
			dwStatus = g_ITEAPI_TxReadRegOFDM((Word)regAddr, (Byte*) &vaule, DevNo);
			if (dwStatus)
				printf("g_ITEAPI_TxReadRegOFDM() return error!\n");
			else
...
static uint16_t Tx_EEPROM_checksum(uint8_t CFG_Length, uint8_t *image)
{
	uint16_t sum = 0;
	int i = 0;
...
int Tx_EEPROM_write (int DevNo, uint8_t *eep_tmpbuf)
{	
	uint16_t sum = 0;	
	int i = 0;	
	...
	sum = Tx_EEPROM_checksum(CFG_Length, eep_tmpbuf);	
	eep_tmpbuf[0] = (uint8_t) (sum >> 8);	
	eep_tmpbuf[1] = (uint8_t) sum;	
	
	for (i = 0; i < 256; i++) {
		//memcpy(request.buffer, &EepromData[i], 32);	
		error = g_ITEAPI_TxWriteEEPROM(i, eep_tmpbuf[i], DevNo);
		if (error)	{
			printf("g_ITEAPI_TxReadEEPROM write fail. Code:[%u]", error);
			return -1;
		}
	}	
...
int Tx_EEPROM_read(Byte DevNo, uint8_t *eep_tmpbuf)
{	
	uint16_t sum = 0, check = 0;	
	int i = 0;	
	uint8_t CFG_Length = 0;	
	Byte buffer[2];
	Word error;
	...
	for (i = 0; i < 256; i ++) {	
		error = g_ITEAPI_TxReadEEPROM(i, buffer, DevNo);
		if (error)	{
			printf("g_ITEAPI_TxReadEEPROM read fail. Code:[%u]", error);
			return -1;
		}
		memcpy(eep_tmpbuf+i, buffer, 1);
	}	

	//checksum	
	CFG_Length = eep_tmpbuf[2];
	sum = Tx_EEPROM_checksum(CFG_Length, eep_tmpbuf);	
	check = (eep_tmpbuf[0]<<8 | eep_tmpbuf[1]);
	...
	if (sum != check) {	
		...
		// update checksum value	
		sum = Tx_EEPROM_checksum(CFG_Length, EEPROM_Default_Data);	
		eep_tmpbuf[0] = (uint8_t) (sum >> 8);	
		eep_tmpbuf[1] = (uint8_t) sum;

		for (i = 0; i < 256; i++) {
			memcpy(eep_tmpbuf+i, &EEPROM_Default_Data[i], 1);	
			error = g_ITEAPI_TxWriteEEPROM(i, eep_tmpbuf[i], DevNo);
			if (error)	{
				printf("g_ITEAPI_TxReadEEPROM write fail. Code:[%u]", error);
				return -1;
		...
		printf("Default EEPROM update finished!!\n");	
	}	

	printf("\n+++++++++++++ Current EEPROM Info +++++++++++++\n");	
	printf(" Tuner ID: %d (decimal number)\n",eep_tmpbuf[EEPROM_TUNERID_ADDRESS]);	
	printf(" Device Type:%d (decimal number)\n",eep_tmpbuf[EEPROM_DEVICETYPE_ADDRESS]);	

	return 0;	
}

int Tx_EEPROM_init(Byte DevNo, uint8_t *eep_tmpbuf)
{
	/**********************************************************	
	...
	*********************************************************/	
	printf("Read EEPROM info\n");
	if(Tx_EEPROM_read(DevNo, eep_tmpbuf))
	{	
...
int Tx_EEPROM_raw_data_editor(Byte DevNo)	
{
	int mene_flag = 1, res = 0, select_fun = 0;	
	uint8_t eeprom_tmpbuf[256];	
	Word bChangeTunerId = 0;

	memset(eeprom_tmpbuf, 0x00, sizeof(eeprom_tmpbuf));
	if(Tx_EEPROM_init(DevNo, eeprom_tmpbuf))
	{
		printf("Tx_EEPROM_init failed\n");
		return -1;
	}
	...
	while(mene_flag)
	{
		...
		switch(select_fun)
		{
			case 1:// Read EEPROM
				memset(eeprom_tmpbuf, 0x00, sizeof(eeprom_tmpbuf));
				if(Tx_EEPROM_read(DevNo, eeprom_tmpbuf))
				{
					printf("Omega_EEPROM_read failed\n");
					return -1;
				}
			...
			case 2:// Write EEPROM
				...
				//--- Tuner ID Number ---	
				printf("\nCurrent Tuner ID: %d(decimal number)", eeprom_tmpbuf[EEPROM_TUNERID_ADDRESS]);	
				...
				//--- Device Type ---	
				printf("\nCurrent Device Type: %d(decimal number)", eeprom_tmpbuf[EEPROM_DEVICETYPE_ADDRESS]);	
				printf("\nChange Device Type? (yes:1, no:0): ");	
				res = 0;	
				res = scanf("%hu", &bChangeTunerId);	
				if(bChangeTunerId != 1) bChangeTunerId = 0;	
				else	
				{	
					printf("Please key in the new Device Type(decimal number):");
					res = 0;	
					res = scanf("%hhu",&eeprom_tmpbuf[EEPROM_DEVICETYPE_ADDRESS]);	
				}	
				...
				{
					if(Tx_EEPROM_write(DevNo, eeprom_tmpbuf))
					{	
						printf("Tx_EEPROM_write failed\n");	
						return -1;	
					}	
				}	
				break;	
...
*/
			...
			case 9:
				...
				break;
			case 10:
				ret = Tx_EEPROM_raw_data_editor(DevNo);
				if(ret != 0)
					printf("Set IT950x EEPROM error(0x%X)\n", (Byte) ret);
				break;
			case 11:
				isInversion = 0;
